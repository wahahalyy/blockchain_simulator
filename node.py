...
def auto_mining(miner_wallet_address):
    ...
    while True:
        ...
        try:
            if blockchain.mempool.size() == 0:
                consecutive_empty_blocks += 1
                if consecutive_empty_blocks > 3:  # 连续3次空区块后延长等待
                    time.sleep(30)
                else:
                    time.sleep(10)
                continue
            ...
            consecutive_empty_blocks = 0
            logger.info(f"开始自动挖矿，内存池中有 {blockchain.mempool.size()} 笔交易")
            block = blockchain.mine(miner_wallet_address)
            if block:
                logger.info(f"自动挖矿成功，新区块: {block.index}, 包含 {len(block.transactions)-1} 笔交易")
