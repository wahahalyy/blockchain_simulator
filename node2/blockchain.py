...
class Blockchain:
    ...
    # 在 blockchain.py 的 mine 函数中，确保即使没有交易也能正常挖矿
    def mine(self, miner_address=None):
        ...
        # 记录是否有普通交易
        has_regular_transactions = len(transactions) > 0
        ...
        block_type = "空块" if not has_regular_transactions else "普通块"
        print(f"{block_type}挖出！索引: {new_block.index}, 哈希: {new_block.hash[:16]}...")
